...
const addParticipant = () => {
  ...
  alert(name)
  if (names.length !== "") {
    errorMessage.value = 'Enter a valid name';
    return;
  }
...
const addParticipant = () => {
  ...
  if (names.includes('')) {
    errorMessage.value = 'Enter a valid name';
    // errorMessage.value = 'All names must be filled out.';
    nameErrors.value = names.map(name => name === '');
    return;
  }
...
const handleSubmit = async () => {
  nameErrors.value = [];
  const names = participants.value.filter(p => p.name !='');
  if (names.includes('')) {
    errorMessage.value = 'All names must be filled out.';
    nameErrors.value = names.map(name => name === '');
    return;
...
.rcontainer {
  ...
  border-radius: 8px;
  /* box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
  border: 5px solid rgb(123, 106, 106); */
